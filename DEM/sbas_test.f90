...
PROGRAM sbas_test
  ...
  IF(iargc().lt.5)THEN
     WRITE(*,*)'usage: sbas_test1 flist nfiles nslcs len looks <r_ref=len/2> <az_ref=lines/2>'
     STOP
  END IF
...
10 continue
  ...
  if(iargc().ge.6)then
     CALL getarg(6,str)
     READ(str,*)r_ref ! ref pix in range
  end if
  if(iargc().ge.7)then
     CALL getarg(7,str)
     READ(str,*)az_ref ! ref pix in azimuth
  end if
  ...
  DO i=1,ncells
     ...
     coh(:,:,i)=dat((nr+1):2*nr,:)
  END DO
  PRINT*,'Correlations read in, nr, naz.'

  !read in unwrapped igrams
  DO i=1,ncells
     strint=cells(i)
     strunw=strrep(strint,'.f','.u')
     OPEN(UNIT=22,FILE=strunw,FORM='unformatted',ACCESS='direct',RECL=2*nr*naz*4,CONVERT='LITTLE_ENDIAN')  
     READ(22,rec=1)dat
     CLOSE(22)
  ...
  DO i=1,ncells
     ...
     READ(23,rec=1)dat
     CLOSE(23)
     amp=amp+dat(1:2*nr-1:2,:)+dat(2:2*nr:2,:)
  END DO
...
!at each pixel, solve for velocity at (nslc-1) time interval
  ...
  open(99,file='Tminv')
  do k=1,ncells
     write(99,*)(Tminv(k,kk),kk=1,nslc-1)
  end do
  close(99)
  ...
  DO j=1,naz_s
     DO i=1,nr_s
        temp1=phase_s(i,j,:)-phase_s(r_ref,az_ref,:)
        temp1=temp1(:)
...
!Create stack to plot images (using MATLAB)
  ...
  OPEN(29,FILE='stack',FORM='unformatted',ACCESS='direct',RECL=nr_s*naz_s*8)
