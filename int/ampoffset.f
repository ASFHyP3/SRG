...
c     modified 09may06 to use start, stop, number of locations down
c     modified 09may06 to fix off by 1 location error
...
c  run silent ?
 ...
 20   if(talk.eq.'y')print '(a,$)','complex data filename 2: '
     ...
     +     status='unknown')
      ...
      if(talk.eq.'y')print '(a,$)','file 1 init. down, final down, # of down locations: '
      read(*,*)isamp_sdn,isamp_fdn,nlocdn
      dsampdn=float(isamp_fdn-isamp_sdn)/float(nlocdn-1)
      if(talk.eq.'y')write(*,*)'down step size: ',dsampdn

      if(talk.eq.'y')print *
      do j=0,nlocdn-1
         ilin1(j)=isamp_sdn+nint(j*dsampdn)
         if(talk.eq.'y')print'(a,i3,a,i5)','file 1 location down ',j+1,': ',ilin1(j)
      end do
      ...
      do idnloc=0,ndnloc-1
         ...
         do n=1,nloc
            ...
            do j=1,NPTS         !read input data (stationary part)
               ...
               do i=1,NPTS
                  a(i,j)=temp(i-1+(n-1)*dsamp+isamp_s)
c                  write(*,*)a(i,j)
               end do
            end do
...
c     read in file 2 data (twice as much)
            ...
            do j=1,NPTS*2
               ...
               do i=1,NPTS*2
                  b(i,j)=temp(i-1-NPTS/2+(n-1)*dsamp+isamp_s+ioffac)
               end do
            end do
c     estimate and remove the phase carriers on the data
...
c            write(*,*)'quadratic offac, offdn: ',offac,offdn
            ...
            if(talk.eq.'y')write(*,*)'Interpolated across peak at ',offac+ioffac+(ipeak-1)/2.
            if(talk.eq.'y')write(*,*)'Interpolated down peak at   ',offdn+ioffdn+(jpeak-1)/2.
            if(talk.eq.'y')write(*,*)'SNR: ',snr
           write(31,'(1x,i6,2x,f12.4,2x,i6,2x,f12.4,2x,f8.3)')
     +            (n-1)*dsamp+isamp_s,offac+ioffac+(ipeak-1)/2.,
     +            ilin1(idnloc),offdn+ioffdn+(jpeak-1)/2.,snr
