!c  geo_interpolate : interpolate a new geo file from two existing ones

      complex*8, allocatable:: in1(:,:),in2(:,:),out(:,:)
      ...
      if(iargc().lt.7)then
         write(*,*)'usage: geo_interpolate infile1 infile2 outfile jd1 jd2 jdout length <valid_lines>'
         stop
      end if
      ...
      allocate (in1(na,nd), in2(na,nd), out(na,nd))
