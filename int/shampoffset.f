...
c     original code written by Howard Zebker
c     modified by Sang-Ho Yun,  April 19, 2006
c       - silent mode default
c       - Down locations don't have to be specified.
c         Simply provide iline_s iline_f ndnloc just like in across case
c       - Box location used to be 1 pixel shifted --> fixed
c       - The problem of the origin location after oversampling (factor 16) was fixed
c       - slight improvement on readability
      ...
      integer dsamp,iac(1:10000),idn(0:10000)
...
c  run silent ?
      ...
      if(talk.eq.'y')write(*,*)'**** Amplitude cross-correlation ',
     +                         'offsets ****'
 ...
 20   if(talk.eq.'y')print '(a,$)','complex data filename 2: '
     ...
     +     status='unknown')
      ...
      if(talk.eq.'y')print '(a,$)','file 1 init. across, final across,',
     +                     '# across locations: '
      read(*,*)isamp_s,isamp_f,nacloc
      dsamp=float(isamp_f-isamp_s)/float(nacloc-1)
      do iacind=1,nacloc
         iac(iacind)=isamp_s+(iacind-1)*dsamp
      end do
      if(talk.eq.'y')write(*,*)'across step size: ',dsamp
      if(talk.eq.'y')print '(a,$)','file 2 approximate integer offsets',
     +                     'in across, down: '
      read(*,*)ioffac,ioffdn
      
      if(talk.eq.'y')print '(a,$)','number of down locations: '
      read(*,*)iline_s,iline_f,ndnloc

      dline=float(iline_f-iline_s)/float(ndnloc-1)
      do idnind=1,ndnloc
         idn(idnind)=iline_s+(idnind-1)*dline
      end do
      
c      if(talk.eq.'y')print *
c      do j=0,ndnloc-1
c         if(talk.eq.'y')print'(a,i3,a,$)','file 1 location down ',j+1,': '
c         read(*,*)idn(j)
c      end do
      
      if(talk.eq.'y')print '(a,$)','minimum required SNR to save ',
     +                     'offset data: '
      read(*,*)snr_min
      
      do idnind=1,ndnloc
         if(talk.eq.'y')print *
         if(talk.eq.'y')write(*,*)'down file 1: ', idn(idnind)
...
c     read in file 2 data (twice as much)
            ...
            irec=idn(idnind)+ioffdn-NPTS/2-1 !offset in down
            do j=1,NPTS*2
               read(unit=22,rec=irec+j)(temp(k),k=1,len)
               do i=1,NPTS*2
                  b(i,j)=temp(i-1+iac(iacind)-NPTS/2+ioffac)
               end do
            end do
c     estimate and remove the phase carriers on the data
...
c  print out absolute correlations at original sampling rate
            if(talk.eq.'y')write(*,*)'Absolute offsets, original ',
     +                               'sampling interval:'
            do kk=-NDISP*2,NDISP*2,2
               if(talk.eq.'y')print '(1x,17i4)',(ic(k,kk),
     +                                          k=-NDISP*2,NDISP*2,2)
            end do
            if(talk.eq.'y')print *
            if(talk.eq.'y')write(*,*)'Expansion of peak, sample ',
     +                               'interval 0.5 * original:'
            do kk=jpeak-NDISP,jpeak+NDISP
               if(talk.eq.'y')print '(1x,17i4)',(ic(k,kk),
     +                                       k=ipeak-NDISP,ipeak+NDISP)
            end do
            if(talk.eq.'y')print *
...
c  load corr with correlation surface
            do ii=1,8
               do jj=1,8
                  corr(ii,jj)=cmplx(c(ipeak+ii-5,jpeak+jj-5),0.)
               end do
            end do
...
c            write(*,*)'quadratic offac, offdn: ',offac,offdn
            ...
            if(talk.eq.'y')write(*,*)'Interpolated across peak at ',
     +                                offac+ioffac+(ipeak-1)/2.
            if(talk.eq.'y')write(*,*)'Interpolated down peak at   ',
     +                                offdn+ioffdn+(jpeak-1)/2.
            if(talk.eq.'y')write(*,*)'SNR: ',snr
           write(31,'(1x,i6,2x,f12.4,2x,i6,2x,f12.4,2x,f8.3)')
     +            iac(iacind),offac+ioffac+(ipeak-1)/2.,
     +            idn(idnind),offdn+ioffdn+(jpeak-1)/2.,snr
