...
for i in range(nfiles-2):
    ...
    command = '$PROC_HOME/int/triplemul '+file1+' '+file2+' '+file3+' '+date1+date2+date3+'.int '+date1+date2+date3+'.amp '+length+' '+lines+' 0 '+looksac+' '+looksdn    

#    command = '$PROC_HOME/int/triplet.py '+file1+' '+file2+' '+file3+' '+length+' '+lines+' '+scale+' '+looksac+' '+looksdn
    print (command)
    ret=os.system(command)
    #  cc files for unwrapping weights
    command = '$PROC_HOME/int/makecc '+date1+date2+date3+'.int '+date1+date2+date3+'.amp '+date1+date2+date3+'.cc '+str(int(int(length)/int(looksac)))+' '+str(int(int(lines)/int(looksdn)))
    print (command)
    ret = os.system(command)
...
if unw == 'y':
    command = '$PROC_HOME/util/unwrap_parallel.py '+str(int(int(length)/int(looksac)))+' '+'tripleintlist'
    print (command)
    ret = os.system(command)
