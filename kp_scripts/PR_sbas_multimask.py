...
#
#
#  PR_sbas_multimask.py -- Given a series of .geo files, generate SBAS time series with basic workflow
...
# 4. Make All Similarity Mask for reference pixels, and all similarity masks for each geocoded SLC, if it hasn't already been made
suff = '_'+max_tb
if os.path.isfile('pssim'+suff)==False:
...
print('\n  Making similarity masks for individual scene dates, if they have not already been formed')
flag = '0'
if INCONSISTENT == 'Y':
    flag = '1'
with open('geolistTEMP','w') as fgeo:
    for geo in geos:
        date = geo[:8]
        if os.path.isfile('cosine_sim/all_similarity_mask_'+date)==False:
            fgeo.write(geo+'\n')
        else:
            command = 'cp cosine_sim/all_similarity_mask_'+date+' .'
            ret=os.system(command)
...
command = '$PROC_HOME/kp_scripts/ps/make_simmasks_mask_parallel.py '+unwwidth+' geolistTEMP '+scenemask+' '+flag+' '+psthresh+' '+simthresh
print('    '+command)
ret=os.system(command)


# 5. Phase Reconstruction for the interferograms using the similarity mask
print('\n  Phase Reconstruction with interferogram-specific similarity masks')
...
command = '$PROC_HOME/kp_scripts/ps/psfilter_kp.py intlistTEMP'+' '+unwwidth
print('    '+command)
ret=os.system(command)
