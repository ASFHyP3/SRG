...
#
#
#  original_sbas.py -- Given a series of .geo files, generate SBAS time series with basic workflow
...
looksac = sys.argv[5]
looksdn = sys.argv[6]
scenemask = sys.argv[7]
INCONSISTENT = sys.argv[8]
psthresh = sys.argv[9]
simthresh = sys.argv[10]
refthresh = sys.argv[11]
TROPO_CORR = sys.argv[12]
...
if TROPO_CORR == 'Y':
    ...
    with open('unwlist_'+max_tb,'w') as funw:
        for name in ints:
            unwfile = '../'+name.replace('.int','.unw')
            funw.write(unwfile)

    # Run the regression
    command = '$PROC_HOME/kp_scripts/util/regress_igrams.py unwlist_'+max_tb+' ../dem '+unwwidth+' '+unwlength+' ../ref_locs_'+max_tb+' '+INCONSISTENT
    print('    '+command)
    ret=os.system(command)
...
if TROPO_CORR == 'Y':
    ...
    for unw in unwfiles:
       command = 'mv ../'+unw.replace('.unw','.amp')+' .'
       ret = os.system(command)
       if INCONSISTENT=='Y':
           command = 'mv ../'+unw.replace('.unw','.mask')+' .'
           ret=os.system(command)    
...
if TROPO_CORR == 'Y':
    os.system('mv *.amp *.mask ..')
    os.chdir('..')

print('\n  !!!!! ----- ORIGINAL SBAS WORKFLOW COMPLETE ----- !!!!!')
