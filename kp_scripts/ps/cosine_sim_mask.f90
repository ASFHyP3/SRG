...
! Compile as: gfortran -o cosine_sim_mask cosine_sim_mask.f90 -fopenmp
! $PROC_HOME/fft/fftw-3.3.9/.libs/libfftw3f.a

PROGRAM cosine_sim_mask
  use omp_lib

  IMPLICIT none

  !!!!! ----- Specifications: Declare your variables ----- !!!!!
  INTEGER::i,j,r,c,rows,cols,stat,fstat,ierr,looks,k,kk,n_refs,igramnumber,iter,maskflag
  ...
  CHARACTER(200)::filelist,str,ref_locs_file, outfile, scenemaskfile, suffix
  CHARACTER(200),DIMENSION(:),ALLOCATABLE::cells !array of file names
  CHARACTER(200)::strint,strunw,stramp,strcc,strmask
  ...
  integer*1,dimension(:,:),allocatable :: bytemask,tempmask,scenemask
  integer*1,dimension(:,:,:),allocatable :: masks

  !!!!! ----- Executions ----- !!!!!

  ! filelist: list of wrapped intrerferogram files, typically ending in *.int
  ! len: width of the files
  ! outfile: name of output all_similarity_mask
  ! scenemaskfile: name of input scenemask
  ! maskflag: Determines if individual interferogram files are loaded in (inconsistent SLC coverage)
  !        0=NO individual interferogram files needed
  !        1=individual interferograms masks are loaded in 
  ! <psthresh>: optional, threshold value for designating PS pixels from MLEstimator
  ! <simthresh>: optional, threshold cosine similarity value for designating PS pixels from cosine similarity index
  ! <suffix>: optional, tag string onto end of output files, to be more specific

  IF(iargc().lt.3)THEN
     WRITE(*,*)'usage: cosine_sim_mask igramlist len outfile scenemaskfile maskflag <psthresh=2> <simthresh=0.5> <suffix=''>'
     STOP
  END IF
  ...
  if(iargc().ge.8)then
     call getarg(8,suffix)
  end if

  !!!!! ----- Determine the number of interferograms in igramlist ----- !!!!!
...
10 continue
  ...
  !!!!! ----- Determine the height of the ints (# of azimuth lines) ----- !!!!!
  ...
  ALLOCATE(igrams(nr,naz,ncells),masks(nr,naz,ncells),scr(nr,naz),similarity(nr,naz),allsimilarity(nr,naz))
  ALLOCATE(amp(nr,naz),bytemask(nr,naz),amps(nr,naz,ncells))

  !read in wrapped igrams
  !$omp parallel do private(strint,strmask,tempmask) shared(igrams,ncells,amps,masks)
  DO i=1,ncells

     ! read in the interferogram
     strint=trim(adjustl(cells(i)))
     strmask=Replace_Text(strint,'.int','.mask')
     ...
     ! If maskflag == 1 then load in each individual interferogram mask. The
     ! name is the interferogram filename with .int replaced by .mask
     if (maskflag==1)then
  ...
  ! Load in the scenemask
  allocate(scenemask(nr,naz))
  print*, 'loading scene mask: ',scenemaskfile
  ! read the mask
  open(unit=11,file=scenemaskfile,access='stream')
  read(11)scenemask
  CLOSE(11)

  !!!!! ----- STEP 1: MLE for Candidate PS Pixels ----- !!!!!
  ! call mlesub to
    ! 1. Filter the interferograms in igrams
    ! 2. Compute signal-to-clutter-ratio for valid points
    ! 3. Free up some memory
  r=nr
  c=naz
  if(maskflag==1)then
      call mlesub_intmask(igrams,masks,scenemask,r,c,ncells,scr)
  else
      call mlesub(igrams,scenemask,r,c,ncells,scr)
  end if
...
!Save nr, naz, nslc, ncells parameters in one file
  OPEN(15,FILE='cosine_sim_parameters' // suffix,STATUS='replace')
  WRITE(15,*) nr, naz, ncells, psthresh, simthresh
  CLOSE(15)

  PRINT*,'Mle pass complete'
  ! save scr for later analysis
  open(15,file='scr_floats' // suffix,access='stream')
  write(15)scr
  close(15)

  open(15,file='scr' // suffix,access='stream')
  do i=1,naz
  ...
  open(15,file='scr_mask' // suffix,access='stream')
  write(15)bytemask
  close(15)
  ...
  ! Use cosine similarity filter to remove false positive PS pixels
  ...
  ! note: phase has been calculated using the filtered interferograms (during
  ! mlesub)
  if(maskflag==1)then
     call median_similarity_intmask(phase,masks,scenemask,scr,r,c,ncells,psthresh,similarity)
  else
     call median_similarity(phase,scenemask,scr,r,c,ncells,psthresh,similarity)
  end if

  ! save median similarity for later analysis
  open(15,file='median_similarity' // suffix,access='stream')
  write(15)similarity
  close(15)
  ...
  open(15,file='median_similarity_mask' // suffix,access='stream')
  write(15)bytemask
  close(15)
  print *,'Saved median similarity mask as median_sim_mask, number, mean= ',k,similaritymean

  !!!!! ----- Step 3. Find Additional PS Pixels with All Similarity ----- !!!!!
  ! scan all points for similarity with refined ps set, adding extra discoveries
  ...
  do iter=1,3
     ...
     if(maskflag==1)then
        call all_similarity_intmask(phase,masks,scenemask,similarity,r,c,ncells,simthresh,allsimilarity)
     else
        call all_similarity(phase,scenemask,similarity,r,c,ncells,simthresh,allsimilarity)
     end if
  end do

  ! save in mht format
  open(15,file=trim(outfile) // suffix,access='stream')
  do i=1,naz
     write(15)amp(:,i),allsimilarity(:,i)
  end do
  ! and also as floats
  open(15,file='all_similarity' // suffix,access='stream')
 ...
 do j=1,naz
  ...
  similaritymean=similaritymean/k
  open(15,file='all_similarity_mask' // suffix,access='stream')
  write(15)bytemask
  close(15)
  print *,'Saved all_similarity mask as all_similarity_mask, number, mean= ',k,similaritymean
  print *,'Fractional coverage: ',float(k)/float(naz)/float(nr)
...
END FUNCTION Replace_Text
  ...
  subroutine mlesub_intmask(igrams,masks,scenemask,len,lines,nigrams,scrout)
!c  mlestack - apply mle estimator to phases from stacked time series of igrams
    ...
    integer*1 masks(len,lines,nigrams),scenemask(len,lines)
    integer*1, allocatable :: inmask(:,:)
...
!c  make a filter directly in frequency domain
      ...
      do i=1,nigrams
         ...
         do k=1,len
            !$omp parallel do private(kk) shared(igramfilt)
            do kk=1,lines
      ...
      !!!!! ----- Calculate SCR at Valid Pixels ----- !!!!!
!c  loop over lines
      !$omp parallel do private(line,in,inmask,csum,i,k,pow,amp,phase,aveph) &
      !$omp private(iph,probmax,krho,prob,kmax,rhoest,screst,out) &
      !$omp private(nvalid,valid,thresh,avemag) & 
      !$omp shared(lines,nigrams,len,pi,ixbins,pdfs,igrams,masks,scrout,scenemask)
      do line=1,lines
        if(mod(line,1000).eq.1)print *,'At line: ',line
         ...
         do i=1,nigrams
            in(:,i)=igrams(:,line,i)
            inmask(:,i)=masks(:,line,i)
         end do

         !c  loop over pixels in the line
         do k=1,len
            ! if the scenemask at this pixel is not 0 then do the calculation
            if (scenemask(k,line)==0)then
               out(k)=0
               out(k+len)=0
               cycle
            end if
            ...
            nvalid=0
            do i=1,nigrams
               if (cabs(in(k,i)).ge.thresh.and.inmask(k,i).ne.0)then
                  nvalid=nvalid+1
                  valid(nvalid)=in(k,i)
               end if
            end do
      ...
      !$omp end parallel do
      return
    end subroutine mlesub_intmask



    subroutine mlesub(igrams,scenemask,len,lines,nigrams,scrout)
!c  mlestack - apply mle estimator to phases from stacked time series of igrams
...
!c  loop over lines
      ...
      !$omp private(iph,probmax,krho,prob,kmax,rhoest,screst,out) &
      !$omp private(nvalid,valid,thresh,avemag) & 
      !$omp shared(lines,nigrams,len,pi,ixbins,pdfs,igrams,scrout,scenemask)
      do line=1,lines
        if(mod(line,1000).eq.1)print *,'At line: ',line
...
!           print *,'reverse end'
      ...
      subroutine median_similarity_intmask(phase,masks,scenemask,scr,len,lines,nigrams,psthresh,similarity)
        ...
        integer*1 masks(len,lines,nigrams),scenemask(len,lines)
        real*4 rmin, rmax, psthresh, dist, cossim(20), temp_cossim
        integer len,lines,i,j,ii,jj,p,nigrams,nvalidpair,irmin,irmax,neigh,k,boxsize
...
!        print *,'Starting median pass with number of ps = ',k
        ...
        !$omp parallel do private(j,k,p,neigh,cossim,nvalidpair,temp_cossim,dist,ii,jj) &
        !$omp shared(lines,len,boxsize,iindex,jindex,r,irmin,irmax,scr,psthresh,nigrams,similarity,phase,masks,scenemask)
        do i=1,len
           do j=1,lines
              similarity(i,j)=0.
                          ...
                          if(ii.ge.1.and.ii.le.len.and.jj.ge.1.and.jj.le.lines)then  ! in bounds
                             ...
                             if(scr(ii,jj).ge.psthresh)then ! found a neighbor ps candidate
                                ...
                                if(neigh.le.20)then
                                   nvalidpair = 0
                                   temp_cossim = 0.0
                                   !$omp parallel do private(p) shared(masks,phase,temp_cossim,nvalidpair,i,j,ii,jj)
                                   do p=1,nigrams
                                      if(masks(i,j,p).ne.0.and.masks(ii,jj,p).ne.0)then
                                         temp_cossim=temp_cossim+cos(phase(i,j,p)-phase(ii,jj,p))
                                         nvalidpair=nvalidpair+1
                                      end if
                                   end do
                                   !$omp end parallel do
                                   if (nvalidpair>1)then
                                      cossim(neigh)=temp_cossim/nvalidpair
                                   else
                                      neigh=neigh-1
                                   end if
...
!                    print *,i,j,neigh,similarity(i,j),cossim
        ...
        return
      end subroutine median_similarity_intmask

      subroutine median_similarity(phase,scenemask,scr,len,lines,nigrams,psthresh,similarity)
        !c  median similarity of each ps point with neighbors in some distance
        !range
        ! compute median_similarity - for each ps pixel (>th), compute its phase
        ! similarity with
        !  nearby PS and return the median of similarity measurements
        ...
        ! loop over all ps pixels and compute median similarity
        !$omp parallel do private(j,k,p,neigh,cossim,nvalidpair,temp_cossim,dist,ii,jj) &
        !$omp shared(lines,len,boxsize,iindex,jindex,r,irmin,irmax,scr,psthresh,nigrams,similarity,phase,scenemask)
        do i=1,len
           do j=1,lines
              similarity(i,j)=0.
              if (scenemask(i,j)==0)then
              ...
              if(scr(i,j).ge.psthresh)then  ! found an scr ps candidate
                 ...
                 cossim=0.
                 do k=1,boxsize*boxsize
                    if(neigh.lt.20)then ! only continue if less than 20neighbors found
                       ii=i+iindex(k)
                       jj=j+jindex(k)
                       dist=r(k)
                       if(dist.ge.irmin.and.dist.le.irmax)then  ! in ring to check
                          if(ii.ge.1.and.ii.le.len.and.jj.ge.1.and.jj.le.lines)then
                          ! in bounds
      ...
      subroutine all_similarity_intmask(phase,masks,scenemask,similarity,len,lines,nigrams,simthresh,allsimilarity)
        ...
        integer*1 masks(len,lines,nigrams),scenemask(len,lines)
        real*4 rmin, rmax, dist, cossim(20),simthresh, temp_cossim
        integer len,lines,i,j,ii,jj,p,nigrams,nvalidpair,irmin,irmax,neigh,boxsize,k
        integer*4, allocatable :: iindex(:),jindex(:)
        real*4, allocatable :: r(:)
        ...
        !$omp parallel do private(i,k,p,neigh,nvalidpair,temp_cossim,cossim,dist,ii,jj) &
        !$omp shared(lines,len,boxsize,iindex,jindex,r,irmin,irmax,similarity,psthresh,nigrams,allsimilarity,phase,masks,scenemask)
        ...
        return
      end subroutine all_similarity_intmask

      subroutine all_similarity(phase,scenemask,similarity,len,lines,nigrams,simthresh,allsimilarity)
        !  all_similarity - return similarity for all pixels vis a vis ps points
        !   only add newly discovered pixels, do not remove previous ones
        ...
        ! loop over all pixels and find ps
        !$omp parallel do private(i,k,p,neigh,nvalidpair,temp_cossim,cossim,dist,ii,jj) &
        !$omp shared(lines,len,boxsize,iindex,jindex,r,irmin,irmax,similarity,psthresh,nigrams,allsimilarity,phase,scenemask)
