...
! Requires lsq* files in same directory
! Compile as: gfortran -o regressheight regressheight.f90 lsq.o
  ...
  character*300 ufile,demfile,outfile,locationfile,str,maskfile,intfile
  complex, allocatable :: igram(:,:)
  integer*1, dimension(:,:),allocatable :: mask

  if(iargc().lt.6)then
     print *,'Usage: unwrapped_file outfile demfile locations_file len lines <maskfile> <intfile to regress>'
     stop
  end if
  ...
  if(iargc().ge.7)call getarg(7,maskfile)
  if(iargc().ge.8)call getarg(8,intfile)

  ! ALLOCATE VARIABLES
  allocate (unwrapped(2*len,lines), dem(len,lines))
  allocate(mask(len,lines))
  if (iargc().ge.7)then
     ! read in mask file
     open(unit=31,file=maskfile,access='stream')
     read(31)mask
     close(31)
  else
     mask=-1
  ...
  if(iargc().ge.8)allocate (igram(len,lines))

!c  get the locations
  numloc=0
  open(21,file=locationfile)
  do i=1,10000000
     read(21,*,end=99)temploc(1),temploc(2)
     if (mask(temploc(1),temploc(2)).ne.0)then
        numloc=numloc+1
        loc(1,numloc) = temploc(1)
        loc(2,numloc) = temploc(2)
     end if
