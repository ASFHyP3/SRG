...
!  solve for displacements directly rather than velocities
...
PROGRAM sbas
  ...
  REAL,DIMENSION(:,:),ALLOCATABLE::Tm,deltime,Tminv,ident,A,Ainv
  REAL,DIMENSION(:),ALLOCATABLE::Bperp,timedeltas
  INTEGER::r_ref,az_ref !reference pixel location
  REAL,DIMENSION(:,:,:),ALLOCATABLE::velocity,d
  ...
  IF(iargc().lt.4)THEN
     WRITE(*,*)'usage: sbas_disp unwlist nunwfiles nslcs len <ref_locations_file = x/2,y/2>)'
     STOP
  END IF
...
!Create an array with data from Tm.out
  ALLOCATE(Tm(ncells,nslc-1),A(ncells,nslc))
  OPEN(24,FILE='Tm.out',STATUS='old')
  do k=1,ncells
     READ(24,*,IOSTAT=stat)(Tm(k,kk),kk=1,nslc-1)
  end do
  CLOSE(24)
  OPEN(24,FILE='A.out',STATUS='old')
  do k=1,ncells
     READ(24,*,IOSTAT=stat)(A(k,kk),kk=1,nslc)
  end do
  CLOSE(24)
  !print *,Tm
...
!at each pixel, solve for velocity at (nslc-1) time interval
  ALLOCATE(velocity(nr,naz,nslc-1),d(nr,naz,nslc))
  velocity(:,:,:)=0
  d(:,:,:)=0.
  ALLOCATE(Tminv(nslc-1,ncells),Ainv(nslc,ncells))
  ALLOCATE(temp1(ncells))

  Tminv=pinv(Tm,ncells,nslc-1)
  Ainv=pinv(A,ncells,nslc)
...
!     print *,phase_ref,ref_locs(1,i),ref_locs(2,i)
  ...
  DO j=1,naz
     DO i=1,nr
        ...
        temp1=temp1(:)
        d(i,j,:)=MATMUL(Ainv,temp1) 
     END DO
  END DO
  DEALLOCATE(temp1)
! compute velocities from displacements
  do i=1,nslc-1
     velocity(:,:,i)=d(:,:,i+1)-d(:,:,i)
  end do
  !Write displacement matrix into file
  OPEN(28,FILE='d',FORM='unformatted',ACCESS='direct',RECL=nr*naz*nslc*4)
  WRITE(28,rec=1)d
  close(28)
