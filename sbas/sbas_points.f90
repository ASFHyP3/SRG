...
!  points version prints solution only at specified point locations rather than an image

PROGRAM sbas_points
  IMPLICIT none

  !specifications
  INTEGER::i,j,r,c,rows,cols,stat,fstat,ierr,looks,k,kk,n_refs,n_points
  ...
  CHARACTER(100)::filelist,str,ref_locs_file,point_locs_file
  ...
  IF(iargc().lt.4)THEN
     WRITE(*,*)'usage: sbas_points unwlist nunwfiles nslcs len ref_locations_file points_file'
     STOP
  END IF
...
99   n_refs=i-1
  ...
  ! set list of point locations
     CALL getarg(6,str)
     READ(str,*)point_locs_file ! ref pix file with list of locations
     open(unit=3,file=point_locs_file)
     do i=1,100
        read(3,*,end=98)point_locs(:,i)
     end do
98   n_points=i-1
     close(3)
  print *,'Number of points for solution: ',n_points

!  ALLOCATE(coh(nr,naz,ncells))
  ALLOCATE(disp(n_points,nslc-1))
...
!at each pixel, solve for velocity at (nslc-1) time interval
  ALLOCATE(velocity(n_points,nslc-1))
  velocity(:,:)=0
...
!     print *,phase_ref,ref_locs(1,i),ref_locs(2,i)
  ...
  DO j=1,n_points
        temp1=phase(point_locs(1,j),point_locs(2,j),:)-phase_ref(:)
        temp1=temp1(:)
        velocity(j,:)=MATMUL(Tminv,temp1) 
  END DO
  DEALLOCATE(temp1)
  !Write velocity solution into file
  OPEN(28,FILE='velocity_points')!,FORM='unformatted',ACCESS='direct',RECL=nr*naz*(nslc-1)*4)
  do j=1,nslc-1
     write(28,*)j,velocity(1:n_points,j)
  end do
  close(28)

  ! write displacements
  OPEN(29,FILE='displacement_points')!,FORM='unformatted',ACCESS='direct',RECL=nr*8)
  do i=1,nslc-1
     do j=1,n_points
        disp(j,i)=sum(velocity(j,1:i)*timedeltas(1:i))
     end do
  end do
  do i=1,nslc-1
     write(29,*)i,disp(1:n_points,i)
  end do
  close(29)
