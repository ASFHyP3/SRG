...
PROGRAM sbas_refpoints
  ...
  integer, dimension(2,100) :: ref_locs
  ...
  IF(iargc().lt.5)THEN
     WRITE(*,*)'usage: sbas unwlist nunwfiles nslcs len looks <ref_locations_file = x/2,y/2>)'
     STOP
  END IF
...
!  close(99)
  ...
  phase_ref=0.
  do i=1,n_refs
     phase_ref=phase_ref+phase_s(ref_locs(1,i),ref_locs(2,i),:)/n_refs
