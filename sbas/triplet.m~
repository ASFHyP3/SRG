...
phasemin=-15;   %   for movie scaling aesthetics
phasemax=15;
...
% SBAS least squares
    ...
    r_ref=[475 448 478 452 480 483];   %  reference pixel location
    az_ref=[229 285 308 286 304 241];
...
%     % histogram of all the phases
%     phasehist=zeros(1,1000);
%     phmin=min(min(min(phi)));
%     phmax=max(max(max(phi)));
%     phbins=linspace(phmin,phmax,1000);
%     for i=1:n
%         phasehist=phasehist+hist(reshape(phi(:,:,i),nr*naz,1),phbins);
%     end
%     q=cumsum(phasehist);
%     phimin=phbins(find(q<q(end)*0.01,1,'last'));
%     phimax=phbins(find(q<q(end)*0.95,1,'last'));
