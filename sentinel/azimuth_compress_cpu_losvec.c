				  ...
				  double *prf)
{
  ...
  FILE *fplos; // stream for the file descriptor *fdlos
  ...
  outlos = (double *) malloc(arraysize * sizeof(double) * 3);
  inlos = (double *) malloc(arraysize * sizeof(double) * 3);
  ...
  for (line=firstline; line<lastline;line++){
    ...
    for (ipix=0; ipix<3; ipix++){
      ...
      // KARISSA YOU ARE HERE!!!!!
      // I want the middle unit look vector for the pixel (how do I know which is the middle for the pixel?
      // unitlookvector_save = ?
...
// compute azoff array in cpu
  ...
  // KARISSA, THIS IS WHERE TO WRITE OUT THE LOS VECTOR???
  // what is the format for writing the file? what are these nbytes? what is this stuff? iaddr_off_t, SEEK_SET, iaddr_size_t, indata?
  // Ideally I would write the los vector here?
  // fplos=fdopen(*fdlos,"w+")
  // nbytes=frwite(what? and where?)
