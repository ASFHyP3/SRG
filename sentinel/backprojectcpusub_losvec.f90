!!!!!
!
!  backprojectsub_losvec - subroutine to back project a sentinel burst to a regular grid defined by dem file
!   use gpu for complex integration, and also writes an LOS file
!
!   steps:
...
!    5. Back project to each point in the dem and write the los_vec file
!
!!!!!

subroutine backprojectcpusub_losvec(rangeprocdata, rangesamples, linestotal, nbursts, lines, slcoutfile, SAFEname, swath, losoutfile)
...
!!!!Image limits
  ...
  real*8 rngstart, rngend, rngpix, latlons(4), maxlatlon
...
!c open the output los vector file
  fdlos=initdk(32,losoutfile)
...
!!!!Initialize satellite position
  ...
  call azimuth_compress_cpu_losvec(rangeprocdata(1,ioffset),satloc,rawdatalines,rangesamples,demwidth,demlength,fdout,fddem,fdlos, &
...
return
end subroutine backprojectcpusub_losvec
