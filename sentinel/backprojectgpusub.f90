...
!c open the output file
  ...
  ! skip burst if bounds not in scene
  maxlatlon=max(abs(latlons(1)),abs(latlons(2)),abs(latlons(3)),abs(latlons(4)))
  if(maxlatlon.le.1000.d0)then
!!!!Initialize satellite position
     ...
     call azimuth_compress(rangeprocdata(1,ioffset),satloc,rawdatalines,rangesamples,demwidth,demlength,fdout,fddem, &
  ...
  end if  !  end bounds test
end do  ! end bursts loop

return
end subroutine backprojectgpusub
