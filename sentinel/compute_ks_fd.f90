...
!
!  compare clutterlock result with elevation steering commands
...
!!$  print *,'fdc ',fdc
  ...
  open(31,file='azibeam.out')
     do i=1,lines
        ...
        fdop(i)=2.*7000./0.06*ang(i)*pi/180.
        prf(i)=fref/in3(b(1+50,i))*1.e6
        t(i)=(i-lines/2.)/prf(i)
        write(31,*)i,azimuthbeam,ang(i),fdop(i),t(i)
     end do
     close(31)
     call linfit(t(21),fdop(21),lines-40,azc0,azc1)
     azks=azc1
     print *,'Azimuth fit coeffs, azks: ',azc0,azc1
     azfdfit=(azc0+lines/2*azc1)
     azfdc=mod(azfdfit,burstprf)
     if(azfdfit.le.0)then
        if(azfdc.le.-burstprf/2.)azfdc=azfdc+burstprf
     end if
     if(azfdfit.ge.0)then
        if(azfdc.ge.burstprf/2.)azfdc=azfdc-burstprf
     end if

     print *,'Az beam ks, fd: ',azks,azfdc

     print *,'Max min times, angles, fdop: ',t(1),t(lines),ang(1),ang(lines),fdop(1),fdop(lines)

!  solve for angle as functiom of time
     call linfit(t(21),ang(21),lines-40,angc0,angc1)
     print *,'Angle coeffs: ',angc1,angc0
     ...
     pi=3.14159265359
     fd0=-10000.
     time=(r*wvl/2/v/v*fd0-r*angc0*pi/180./v)/(1.+r*angc1*pi/180./v)
     print *,'fd0,time: ',fd0,time
     fd0=0.
     time=(r*wvl/2/v/v*fd0-r*angc0*pi/180./v)/(1.+r*angc1*pi/180./v)
     print *,'fd0,time: ',fd0,time
     fd0=10000.
