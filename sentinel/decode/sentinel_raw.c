...
#pragma omp parallel for shared(recs,iptr,map,times,zeros,line10,line11,line12,raw1,raw2,raw3) private(packetDataLength,sigtyp,swath,nq,j,time)
  ...
  // sort by burst and range compress
  
  
}

void sortswath(unsigned char *raw, long int lines){

  // decode a few parameters needed for range compression
  rangedecimation=raw[40]&&255;
  samplefreq=samplefrequency[rangedecimation];
  ipolarity=(int2(raw[42])&&32768)/32768;
  ramprate=(-1)**(1-ipolarity)*(int2(raw[42])&&32767)*fref*fref/pow(2,21);
