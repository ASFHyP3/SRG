...
void decode_line_(unsigned char *data, int *nqptr, int *packetDataLengthptr, float *samples, int *linenumptr){

  int b, brc[500], thidx[500], temp, bits, startbyte, signbit, bitstoshift, block;
  int nblocks, i, sampindex, sampstodecode,nq;
  long q;
  double value;

  // print input params
  nq=*nqptr;
  printf("nq nqptr packetDataLength linenum %d %d %d %d\n",nq,*nqptr,*packetDataLengthptr,*linenumptr);
