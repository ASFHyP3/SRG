!!!!!
!
!  geo2rdr_reramp - resample a sentinel slc to a regular grid defined by a dem file
!     for zero Doppler slc geometry only, add the deramp phase back in
!
!
!!!!!

program geo2rdr_reramp
  use sql_mod
  use omp_lib
  implicit none
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  integer*4 firstValidSample(100), lastValidSample(100)
...
!  integer :: numOutsideImage
  ...
  real*8, allocatable :: derampphase(:,:)
...
!!$  integer :: inta, intr
  ...
  if(iargc().lt.2)then
     print *,'usage: geo2rdr_reramp file_db slcoutfile ' !posting(pts per dem point) <input_file=db_slcfile>'
     stop
  end if
...
!!$  read(str,*)ptsperdempt
  ...
  do burst=1,azimuthBursts
     if(burst.le.9)then
        ...
        call get_parami(db,table,'firstValidSample'//char(48+burst),firstValidSample(burst),units,type)
        call get_parami(db,table,'lastValidSample'//char(48+burst),lastValidSample(burst),units,type)
     else
        ...
        call get_parami(db,table,'firstValidSample'//'1'//char(48+burst-10),firstValidSample(burst),units,type)
        call get_parami(db,table,'lastValidSample'//'1'//char(48+burst-10),lastValidSample(burst),units,type)
     end if
  end do
...
!  call get_paramc(db,table,'orbinfo',orbfile,units,type)
  ...
  allocate(burstdata(samplesPerBurst,linesPerBurst), derampphase(samplesPerBurst,linesPerBurst))
...
!!$  open(32, file='azimuth',access='direct',recl=4*demwidth,form='unformatted')
  str=dbfile(1:index(dbfile,'db.')-1)
  swathnum=dbfile(index(dbfile,'db.')+3:index(dbfile,'db.')+3)
!!$  print *,slcoutfile
...
!!$  print *,'dbfile ',dbfile
!!$  print *,'swathnum: ',swathnum
!!$  print *,'str ',str
!!$  print *,'slcoutfile',slcoutfile
  if(swathnum.eq.'1')then  ! if on first swath then initialize
...
!!$     print *, 'Radar Image Burst Width: ', width
     ...
     ! read in the deramp function
     open(21,file='deramp_phase',access='direct',recl=8*samplesPerBurst*linesPerBurst)
     read(21,rec=burst)derampphase
     close(21)
...
!                 print *,xyz,tline,rngpix
              ...
              call resample2grid_burst_reramp(rgoff,azoff,rgm,burstdata,derampphase,demwidth, &
                   samplesPerBurst,linesPerBurst, &
                   firstValidLine(burst),lastValidLine(burst), &
                   firstValidSample(burst),lastValidSample(burst),wvl,outdata)
