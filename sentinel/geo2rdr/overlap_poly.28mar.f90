!
!  overlap_poly get polynomial fit to phase in overlapped region
!
  IMPLICIT NONE

  REAL*8    :: xin(100000),yin(100000), center, sum,sumsq
  complex*8 :: data(100000),csum, c, offset
  complex*8 :: burstline(1000),gradient(999),correction(1000), avegradient
  complex*8, allocatable :: deramp(:,:), flat(:,:), bdata(:,:), burstdata(:,:)
  INTEGER   :: i, ndata, k, burst, istart, nextnonzero, line
  integer   :: len, overlap, bursts, firstgood, lastgood, rangelooks
  integer*2 :: hist(21)
  REAL      :: poly(2), phase, phaseac,deltaphase,phases(1000),deltas(999),filtered(1000)
  real      :: meandelta, mediandelta, median
  real      :: burstphase(1000),phmin,phmax,mindiff,bestcentroid,centroid
  real      :: phmean,ph2nd,phstd
  real, allocatable :: testphases(:,:),testphases2(:,:)
  character*300 :: str,burst_image_file

  if(iargc().lt.6)then
     print *,'usage: overlap_poly burst_image_file len overlap bursts firstgoodline lastgoodline'
     call exit
  end if

  call getarg(1,burst_image_file)
  call getarg(2,str)
  read(str,*)len
  call getarg(3,str)
  read(str,*)overlap
...
! allocate arrays
  allocate(burstdata(len,overlap),bdata(1000,overlap),deramp(1000,overlap),flat(1000,overlap))
  allocate(testphases(1000,bursts),testphases2(1000,bursts))

  open(22,file='testphaseimage',access='direct',recl=1000*8)

  open(21,file=burst_image_file,access='direct',recl=8*overlap*len)
  open(31,file='burst_start_stop_phases')
  open(23,file='bdata',access='direct',recl=8*overlap*1000)
  open(25,file='qderamp',access='direct',recl=8*100*1000)
!  open(24,file='phases')

  !   how many rangelooks to use?
  rangelooks=len/1000
  ...
  do burst=1,bursts-1
     ...
     !  zero out bad lines
     burstdata(:,1:firstgood-1)=cmplx(0.,0.)
     burstdata(:,lastgood+1:overlap)=cmplx(0.,0.)
     ! take looks across
     do line=firstgood,lastgood
        do i=1,1000
           csum=cmplx(0.,0.)
           do k=i*rangelooks-rangelooks+1,i*rangelooks !i*21-20,i*21
              csum=csum+burstdata(k,line)
           end do
           bdata(i,line)=csum+bdata(i,line)
        end do
     end do
     write(23,rec=burst)bdata
     ! take looks down to fill in some more holes
     burstline=sum(bdata(:,:),2)
     testphases2(:,burst)=atan2(aimag(burstline),real(burstline))
     ! find an approximate centroid
     mindiff=1.e10
     bestcentroid=-99
     do i=-30,30
        centroid=i/10.
        burstphase=atan2(aimag(burstline*cmplx(cos(centroid),sin(centroid))) &
          ,real(burstline*cmplx(cos(centroid),sin(centroid))))
        call medfilt(burstphase,filtered,1000)
        phmean=sum(filtered(101:900))/800.
        ph2nd=sum(filtered(101:900)**2)/800.
        phstd=sqrt(ph2nd-phmean**2)
!!$        phmin=minval(filtered(101:900))
!!$        phmax=maxval(filtered(101:900))
        if(phstd.le.mindiff)then
           mindiff=phstd
           bestcentroid=centroid
        end if
        !print *,'centroid, diff : ',centroid,bestcentroid,mindiff
     end do
     burstphase=atan2(aimag(burstline*cmplx(cos(bestcentroid),sin(bestcentroid))) &
          ,real(burstline*cmplx(cos(bestcentroid),sin(bestcentroid))))
     call medfilt(burstphase,filtered,1000)
     filtered=filtered-bestcentroid
     testphases(:,burst)=filtered
     ! fit a polynomial directly
     do i=101,900
        xin(i-100)=i
        yin(i-100)=filtered(i)
     end do
     ndata=800
     call linfit(xin,yin,ndata,poly)
!!$     print *,'Polynomial for direct phases: ',poly
!!$     testphases2(1:999,burst)=filtered
!!$     gradient=burstline(2:1000)*conjg(burstline(1:999))
!!$     deltas=atan2(aimag(gradient),real(gradient))
!!$     meandelta=sum(deltas(101:900))/800.
!!$     mediandelta=median(deltas(101:900),800)
!!$     print *,'Mean and median gradient: ',meandelta,mediandelta
!!$     !  median filter the gradient phases to remove outliers
!!$     call medfilt(deltas,filtered,999)
!!$     testphases(:,burst)=filtered
!!$
!!$     deltas=filtered
!!$     meandelta=sum(deltas(101:900))/800.
!!$     mediandelta=median(deltas(101:900),800)
!!$     print *,'Filtered mean and median gradient: ',meandelta,mediandelta
!!$     ! integrate changes in phase
!!$     phases(1)=deltas(1)
!!$     do i=2,999
!!$        phases(i)=phases(i-1)+deltas(i)
!!$        testphases(i,burst)=deltas(i)
!!$     end do
!!$     ! fit line to integrated phases
!!$     do i=101,900
!!$        xin(i-100)=i
!!$        yin(i-100)=phases(i)
!!$     end do
...
!!$     poly(1)=0.
!!$     poly(2)=meandelta
!!$     print *,'Polynomial: ',poly,poly(1)+1000*poly(2)
     ! compute and apply phase slope correction
     do i=1,1000
        correction(i)=cmplx(cos(poly(1)+i*poly(2)),sin(poly(1)+i*poly(2)))
     end do
     do line=firstgood, lastgood
        deramp(:,line)=bdata(:,line)*conjg(correction)
     end do
     !  write ramp corrected data for check
     write(25,rec=burst)deramp(:,31:130)

     ! now get the offset term and apply that
     offset=sum(sum(deramp(:,firstgood:lastgood),1))
     poly(1)=poly(1)+atan2(aimag(offset),real(offset))
     do i=1,1000
        correction(i)=cmplx(cos(poly(1)+i*poly(2)),sin(poly(1)+i*poly(2)))
     end do
     do line=firstgood,lastgood
        flat(:,line)=bdata(:,line)*conjg(correction)
     end do

     ! write our corrected burst for check
     do i=1,lastgood-firstgood+1
        write(22,rec=(burst-1)*(lastgood-firstgood+1)+i)flat(:,i+firstgood-1)
     end do

     ! write start and stop phases for each overlapped burst
     write(31,*)burst,poly(1),poly(1)+poly(2)*1000.*(float(len)/(1000.*rangelooks))
...
!  close(23)
  close(31)
  open(41,file='testphases')
  do i=1,1000
     write(41,*)i,testphases(i,:)
  end do
  close(41)
  open(41,file='testphases2')
  do i=1,1000
     write(41,*)i,testphases2(i,:)
  end do
  close(41)

end program

    integer function nextnonzero(array,len,istart)
      complex*8 :: array(len)
      do i=istart,len
         if(cabs(array(i)).ge.1.e-2)then
            nextnonzero=i
            return
         else
            nextnonzero=0
...
! Y = a(0) + a(1).X + a(2).X^2 + ... + a(m).X^m
         ...
         REAL*8    :: x(100000),y(100000),xrow(0:20), wt = 1.0, beta(0:20), &
                       var, covmat(231), sterr(0:20), totalSS, center
         real*4  poly(3)
         REAL*8    :: xin(100000), yin(100000)
...
subroutine medfilt(array,out,len)

      real array(len),out(len)
      dimension t(1000)

      nsize=5   !median filter size

      do k=1,len
         out(k)=0.
      end do

      do i=nsize/2+1,len-nsize/2-1
         do ii=-nsize/2,nsize/2
            t(ii+nsize/2+1)=array(i+ii)
         end do
!c  sort
         call sort(nsize,t)
         out(i)=t(nsize/2+1)      
...
real function median(array,len)
