...
!
  ...
  integer maxpts
  parameter (maxpts=30000)
  REAL*8    :: xin(maxpts),yin(maxpts), center, sum,sumsq
  complex*8 :: data(100000),csum, c, offset, rangephase(maxpts)
  complex*8 :: burstline(1000),gradient(999),correction(maxpts), avegradient
  complex*8, allocatable :: phases(:),outphases(:)
  real*8, allocatable :: rangeoffsets(:)
  INTEGER   :: i, ndata, k, burst, istart, nextnonzero, line, linesperburst, kburst
  integer   :: len, overlap, bursts, firstgood, lastgood, rangelooks,listindex
  integer*2 :: hist(21)
  REAL      :: poly(2), phase, phaseac,deltaphase,deltas(999),filtered(1000)
  real      :: meandelta, mediandelta, median
  real      :: burstphase(maxpts),phmin,phmax,mindiff,bestcentroid,centroid
  real      :: phmean,ph2nd,phstd
  real, allocatable :: testphases(:,:),testphases2(:,:)
  character*300 :: str,residual_phase_file,range_offset_file
  integer :: ib(8),ioutlier,samplesperburst
  real :: p1(8),p2(8),sump2,sum2p2,stdvp2

  if(iargc().lt.4)then
     print *,'usage: overlap_poly residual_phase_file range_offset_file len linesperburst samplesperburst'
     call exit
  end if

  call getarg(1,residual_phase_file)
  call getarg(2,range_offset_file)
  call getarg(3,str)
  read(str,*)len
  call getarg(4,str)
  read(str,*)linesperburst
  call getarg(5,str)
  read(str,*)samplesperburst
  call getarg(6,str)
  read(str,*)kburst

! allocate arrays
  allocate(phases(len), rangeoffsets(len), outphases(len))

  open(21,file=residual_phase_file,access='direct',recl=8*len)
  open(22,file=range_offset_file,access='direct',recl=8*len)
  open(25,file='flatresiduals',access='direct',recl=8*len)
  open(31,file='burstpoly')

  print *,'input files opened'

  !c  loop over bursts
  do burst=1,8
     print *,'Starting burst ',burst
     ! make a list of valid phases and their offsets
     listindex=0
     rangephase=cmplx(0.,0.)
     do line=1,linesperburst
        read(21,rec=(burst-1)*linesperburst+line)phases
        read(22,rec=(burst-1)*linesperburst+line)rangeoffsets

        !  find the valid data points and bin in range
        do i=1,len
           if(cabs(phases(i)).ge.1.e-5)then
              k=min(max(nint(rangeoffsets(i)),1),maxpts)
              rangephase(k)=rangephase(k)+phases(i)
           end if
        end do
     end do
     ...
     do i=-30,30
        centroid=i/10.
        burstphase=atan2(aimag(rangephase*cmplx(cos(centroid),sin(centroid))) &
          ,real(rangephase*cmplx(cos(centroid),sin(centroid))))
!        call medfilt(burstphase,filtered,1000)
        phmean=sum(burstphase)/maxpts
        ph2nd=sum(burstphase**2)/maxpts
     ...
     burstphase=atan2(aimag(rangephase*cmplx(cos(bestcentroid),sin(bestcentroid))) &
          ,real(rangephase*cmplx(cos(bestcentroid),sin(bestcentroid))))
!     call medfilt(burstphase,filtered,1000)
     burstphase=burstphase-bestcentroid

     ! create a list of nonzero elements in rangephase
     do i=1,maxpts
        if(cabs(rangephase(i)).ge.1.e-5)then
           listindex=listindex+1
           xin(listindex)=i
           yin(listindex)=burstphase(i)
        end if
     end do

     print *,'Valid points this burst: ',listindex,burst
!!$     print *,xin(1:10)
!!$     print *,yin(1:10)

     if(burst.eq.kburst)then
        print *,'writing points for burst ',kburst
        open(32,file='test')
        do i=1,listindex
           write(32,'(2f10.2)')xin(i),yin(i)
        end do
        close(32)
     end if

     call linfit(xin,yin,listindex,poly)
     print *,'Polynomial for direct phases: ',poly

     ! test by flattening the residual image
     do i=1,maxpts
     ...
     do line=1,linesperburst
        ...
        do i=1,len
           k=min(max(nint(rangeoffsets(i)),1),maxpts)
           outphases(i)=phases(i)*conjg(correction(k))
        end do
        write(25,rec=(burst-1)*linesperburst+line)outphases
     end do

     ! write start and stop phases for each overlapped burst
     write(31,*)burst,poly(1),poly(2)

  end do  ! end burst loop
  close(31)

  !  is there an outlier from a bad fit?
  ioutlier=0
  open(31,file='burstpoly')
  do i=1,8
     read(31,*)ib(i),p1(i),p2(i)
  end do
  close(31)
  sump2=sum(p2)/8
  sum2p2=sum(p2**2)/8
  stdvp2=sqrt(sum2p2-sump2**2)
  do i=1,8
     if(abs(p2(i)-sump2).gt.stdvp2)then
        print *,'outlier ',i
        ioutlier=i
     end if
  end do

  if(ioutlier.gt.0)then
  !  refit the outlier
     rangephase=cmplx(0.,0.)
     do line=1,linesperburst
        read(21,rec=(ioutlier-1)*linesperburst+line)phases
        read(22,rec=(ioutlier-1)*linesperburst+line)rangeoffsets

        !  find the valid data points and bin in range
        do i=1,len
           if(cabs(phases(i)).ge.1.e-5)then
              k=min(max(nint(rangeoffsets(i)),1),maxpts)
              if(k.gt.int(samplesperburst/4).and.k.lt.int(samplesperburst*0.75)) &
                   rangephase(k)=rangephase(k)+phases(i)
           end if
        end do
     end do
...
!     call medfilt(burstphase,filtered,1000)
     ...
     print *,'Valid points this burst: ',listindex,ioutlier

     if(ioutlier.eq.kburst)then
        open(32,file='test2')
        print *,'writing points for burst ',kburst
        do i=1,listindex
           write(32,'(2f10.2)')xin(i),yin(i)
     ...
     do line=1,linesperburst
        ...
        write(25,rec=(ioutlier-1)*linesperburst+line)outphases
     end do

     ! rewrite start and stop phases for each overlapped burst
     open(31,file='burstpoly')
     do i=1,8
        if(i.ne.ioutlier)then
           write(31,*)ib(i),p1(i),p2(i)
        else
           write(31,*)ioutlier,poly(1),poly(2)
        end if
     end do
     close(31)
  end if

  ! finally write the start and stop phases
  open(31,file='burstpoly')
  open(32,file='startstopphases')
  do i=1,8
     read(31,*)ib(i),poly(1),poly(2)
     write(32,*)ib(i),poly(1),poly(1)+samplesperburst*poly(2)
  end do
  close(31)
  close(32)
