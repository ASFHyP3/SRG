...
// malloc cpu arrays
  ...
  llh = (double *) malloc(sizeof(double)*3);
  xyz = (double *) malloc(sizeof(double) * 3 * *demwidth);
  xyztemp = (double *) malloc(sizeof(double)*3);
  satx = (double *) malloc(sizeof(double)*3);
  satv = (double *) malloc(sizeof(double)*3);
  unitlookvector = (double *) malloc(sizeof(double)*3);
...
//  begin loop over lines in dem 
  ...
  for (line=0;line<*demlength;line++){
	...
	for (pixel=0; pixel< *demwidth; pixel++){
	  ...
	  orbitrangetime_(xyztemp,timeorbit,xx,vv,numstatevec,tmid,satx,satv,&tline,&rngpix);
	  ...
	  azoff[pixel]=0;
	  if (tline > *tstart-2. && tline < *tend+2.){
	    if (naperture > *aperture/2 && naperture < *rawdatalines- *aperture/2){
