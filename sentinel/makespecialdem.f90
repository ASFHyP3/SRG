!c  makespecialdem - version of createspecialdem that uses point locations
...
!c   derived from upsampledem - upsample a dem (i*2) file linearly
      ...
      if(iargc().lt.8)then
         print *,'Usage: makespecialdem infile inrscfile outfile ', &
              'outrscfile top-point(y) bot-point left-point(x) ', &
              'right-point <upsampx=1> <upsamppy=upsampx>'
         stop
      end if
...
!c convert points to lats and lons
      toplat=toppoint*ystep+yfirst
      botlat=botpoint*ystep+yfirst
      leftlon=leftpoint*xstep+xfirst
      rightlon=rightpoint*xstep+xfirst
