...
!c   split according to sortbursts.out from sortbursts
!c
  ...
  integer*4 i,j,ipolarity,nlines,npts,nsamps,nvalid,k,kk,kkk
  integer*4 ranfft, rangedecimation, pricount, burst, line
  integer*4 burstno(1000000),burstline(1000000)
  integer in2, in3, in4
...
! loop over each line
  ...
  !$OMP shared(nlines,nsamps,iplanf,iplani,ref,burstno,burstline,nvalid)
  do i=1,nlines
     ...
     !print *,data(1000),burstno(i),burstline(i)
!     print *,'data in ',data(1:10)
     call sfftw_execute_dft(iplanf,data,dataspec)
!     print *,'dataspec 1 ',dataspec(1:10)
     dataspec=dataspec*conjg(ref)  ! multiply by ref
!     print *,'dataspec 2 ',dataspec(1:10)
     call sfftw_execute_dft(iplani,dataspec,data)  ! back to time domain
!     print *,'data out ',data(1:10)
!     print *
     if(burstno(i).ge.1.and.burstno(i).le.11)then
        write(30+burstno(i),rec=burstline(i))in(1:10),data(1:nvalid)
     end if
  end do
  !$OMP end parallel do
