...
if len(sys.argv) < 1:
    print ('Usage: sentinel_cpu(_cloud).py <vv or vh (def. vv)>')
    print
...
for file in os.listdir("."):
    if file.endswith(".list"):
        command = HOME+"/asfdata/asfdownload.py "+file
        print (command)
...
#    create a DEM file if none exists
    ...
    if create_dem==1:
        command=HOME+"/sentinel_coordinates_srtm30.py "+SAFEname
...
#    and process the scene in you have the precise orbit
    ...
    if len(orbitfilename) < 1:
        ...
        command=HOME+'/sentinel_scene_cpu.py '+SAFEname+' '+orbitfilename+' '+pol
