...
proc = subprocess.Popen(HOME+"/bestgpu.sh",stdout=subprocess.PIPE, shell=True)
...
#        command=HOME+"/sentinel_coordinates_srtm30.py "+SAFEname  # nasadem
        command=HOME+"/sentinel_coordinates_copernicus.py "+SAFEname
        print (command)
        ret=os.system(command)
...
#        print (str(orbitfilename.strip().decode()))
        ...
        command=HOME+'/sentinel_scene_gpu.py '+SAFEname+' '+str(orbitfilename,'UTF-8').strip()+' '+pol
