...
int main (int argc, char *argv[]) {
  ...
  long swathswath;
  long long swathptr;
  iptr = (long long *)malloc(100000 * sizeof(long long));
  line = (long long *)malloc(100000 * sizeof(long long));
  whichswath = (long *)malloc(100000 * sizeof(long));
  struct stat sb;
  char fname[200];
  char basename[200];
  char *charerr;
  ...
  // read the swath info
  fpswath=fopen("swath.txt","r");
  printf("swath file opened\n");
  for (i=0;i<recs;i++){
    printf("%d\n",i);
    fscanf(fpswath,"%d %d %d %d %d %d",&swathline,&elevationbeam,&swathswath,&nq,&packetDataLength,&swathptr);
    line[i]=swathline;
    whichswath[i]=swathswath;
    iptr[i]=swathptr;
    printf("%d %d %d %d %d %d\n",line[i],elevationbeam,whichswath[i],nq,packetDataLength,iptr[i]);
  }
  ...
  for (i=0;i<recs;i++){
    ...
    swath=data[64];
    nq=int2(&data[65]);
    //fprintf(fpswath,"%d %d %d %d line elevationbeam swath nq\n",i,elevationbeam,swath,nq);
   ...
   if(sigtyp==0){
     ...
     time[0]=times[i];
     // write header bytes for line, pad to 80 bytes
     /*
     fwrite(&data,1,6+62,fpout);
     fwrite(&time,1,8,fpout);
     fwrite(&zeros,1,4,fpout);
     
     // write unpacked complex samples
   ...
   /*
     ...
     fwrite(&time,1,8,fpout);
     fwrite(&zeros,1,4,fpout);
     fwrite(&zeros,8,30000-80/8,fpout);
     };*/
  }
  fclose(fpout1);
  fclose(fpout2);
