c  acoffoutliers - remove offset outliers from rg offset file
c   filter across only to accommodate variable prfs
...
c  down
      do i=1,nn
         x=(xd(i)-slpdn*cdn+slpdn*dnshift(i))/(1+slpdn**2)
         y=slpdn*x+cdn
         distdn(i)=sqrt((x-xd(i))**2+(y-dnshift(i))**2)
c         print '(3f10.4)',xd(i),dnshift(i),distdn(i)
      end do

      do i=1,nn
c         print *,i,distac(i),distdn(i)
c         if(distac(i).le.distance.and.distdn(i).le.distance)write(21,*)nint(xd(i)),acshift(i),nint(yd(i)),dnshift(i),s(i)
         if(distac(i).le.distance)write(21,*)nint(xd(i)),acshift(i),nint(yd(i)),dnshift(i),s(i)
      end do
