      program extracti2
c  extracti2 - extract a region of i*2 pixels from an image

      implicit none
      integer*2 r_input(100000),r_output(100000)
      integer  i_is,i_ss,i_nl,i_nr,i,j,i_nrec
      character*80 a_input,a_output,c

      if(iargc().lt.6)then
         print *,'usage: extracti2 infile outfile size upperleft x,y xsize (ysize)'
         stop
      end if
      call getarg(6,c)
      read(c,*)i_nr
...
c         print *,'line: ',i
       read(10,rec=i) (r_input(j), j=1,i_nrec)
       do j=i_ss,i_ss+i_nr-1
         r_output(j-i_ss+1) = r_input(j)
       enddo       
       write(11,rec=i-i_is+1) (r_output(j),j=1,i_nr)
