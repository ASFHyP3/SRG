	...
	if(iargc().lt.3)then
	   type *,'usage: fillfloat infile outfile nx'
	   stop
	end if
	...
	call getarg(1,f1)
	call getarg(2,f2)
	open(21,file=f1,form='unformatted',access='direct',recl=nx*4)
	open(22,file=f2,form='unformatted',access='direct',recl=nx*4)
	ierr=fstat(21,istat)
	ny=istat(8)/4/nx
	type *,'Lines: ',ny
