c  fitrg - version of fitrgoffset for use in polinsar script
c
c   gross offset correction added, run without prompts
c
c  fitrgoffset - fit offset output file to range dependence for across, down
c   note: this is different than fitoffset due to file transpose

      parameter (NTERMS=6)	
      parameter (MP=10000)
      parameter (NPP=6)

      character*80 file,str
      ...
      if(iargc().lt.1)then
         write(*,*)'usage: fitrgoffset rgoffsetfile <coarseacc> <coarsedn>'
         stop
      end if
      call getarg(1,file)
      icacc=0
...
c     fit across shifts, 1D across dependence
      ...
      write(*,*)
      write(*,*)'         Slope across  Intercept: '
      write(*,*)'Across: ',slpac,cac+icacc

c     fit down shifts, 1D across dependence
      ma=2
      icoef(1)=1
      icoef(2)=2
      icoef(3)=3
      call svdfit(yd,xd,dnshift,sig,nn,coef,ma,u,v,w,MP,NPP,chisq)

      slpdn=coef(2)
      cdn=coef(1)
      write(*,*)'Down:   ',slpdn,cdn+icdn

      open(31,file='fitrg.out')
      write(31,*)slpac
      write(31,*)cac+icacc
      write(31,*)slpdn
      write(31,*)cdn+icdn
