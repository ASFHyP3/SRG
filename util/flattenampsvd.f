...
c  fit polynomial to brightness

c  get average level
      open(21,file='ave.txt')
      amean=0.
      do i=1,na
         write(21,*)acc(i)
         amean=amean+acc(i)/na
      end do
      close(21)
      print *,'mean',amean
c  get first difference
      first=0.
      do i=1,na-1
         first=first+(acc(i+1)-acc(i))/(na-1)
      end do
      print *,'first',first
c  get second difference
      second=0.
      do i=2,na-1
       second=second+((acc(i+1)-acc(i))-(acc(i)-acc(i-1)))/(na-2)/(na-2)
      end do
      print *,'second',second
...
c  svdfit
      ...
      icoef(5)=5
      icoef(6)=6
      call svdfit(x,y,acc,sig,na,coef,ma,u,v,w,MP,NPP,chisq)
      ...
      do i=1,1200,100
         ai=i
         print *,i,acc(i),coef(1)+ai*coef(2)+ai**2*coef(3)+ai**3*coef(4)
     $         +ai**4*coef(5)+ai**5*coef(6)
      end do

c         write(22,rec=line)(b1(k),k=1,na*2)
 99   continue
      end
	 ...
	 subroutine funcs(x,y,afunc,ma)
        ...
        do i=1,ma
             cf(icoef(i))=1.
c             afunc(i)=cf(6)*(x**2)+cf(5)*(y**2)+cf(4)*x*y+ 
c     +                cf(3)*x+cf(2)*y+cf(1)
             afunc(i)=cf(3)*(x**2)+cf(2)*x+cf(1)+cf(4)*(x**3)+
     +                cf(5)*x**4+cf(6)*x**6
             cf(i)=0.
        end do
