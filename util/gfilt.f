c  gfilt - adaptive filter for interferometric fringes
...
c   from idea of Dick Goldstein in 1997
      ...
      complex indata(8192,size),outdata(8192,size)
...
c  read in size lines
         do i=1,size
            read(21,rec=line+i-1,err=90)(indata(k,i),k=1,len)
            lastline=line+i-1
         end do
...
c  put into output array
            do i=1+iedge,size-iedge
               do j=1+iedge,size-iedge
                  outdata(k+i-1,j)=t(i,j)
               end do
            end do
         end do
c  write to disk
         do i=1+iedge,size-iedge
            write(22,rec=line+i-1)(outdata(k,i),k=1,len)
