c  medfiltcpx - median value filter magnitude of a complex image

      complex in(2048,2048),out(2048)
      integer statb(13),stat
      dimension t(1000)
      character*60 f,ff

      if(iargc().lt.4)then
         print *,'usage: medfiltcpx infile outfile width filt-size(odd)'
         stop
      end if
...
c  put values in a vector
               do jj=-nsize/2,nsize/2
                  k=k+1
                  t(k)=cabs(in(j+jj,i+ii))
               end do
            end do
c  sort
         call sort(nsize*nsize,t)
         out(j)=in(j,i)/cabs(in(j,i))*t(nsize*nsize/2)      
         end do
         write(22,rec=i)(out(k),k=1,npix)
      end do
      do k=1,npix
