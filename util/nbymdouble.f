c nbymdouble -- average a double file n looks across, m looks down
	integer*4 N_RA
	parameter (N_RA = 32768)
	integer*4 initdk, ioread, iowrit, nread, nwr, lun
	integer*4 fdin1, fdin2, fdout
	integer*4 isum(N_RA), width, wido
	real*8      a2(N_RA*64),b(N_RA)
	real*8    sum(N_RA)
	character*120 name
	
	i = iargc()
	if(i .lt. 4) then
	   write(*,*)'usage: nbymdouble infile outfile width acavg [dnmavg]'
	   stop
	end if
