c  shadefloathgt - generate a shaded relief map in mht format from a float image

      parameter (maxsize=5120)
      real*4 indata(maxsize*maxsize),inputline(4*maxsize)
      ...
      if(iargc().lt.2)then
         print *,'usage: shadefloathgt floatimage length <shadescale(1)> <shademean(128)> <firstline> <nlines> <firstpix> <npix>'
         stop
      end if
