c     subset an i*2 image

      implicit none
      integer*2 in(100000),out(100000)
      integer  i,j,len,iac,idn,k,irec,kac,kdn
      character*80 a_input,a_output,c

      if(iargc().lt.7)then
         print *,'usage: subseti2 infile outfile length ac-start dn-start ac-size dn-size'
         stop
      end if
