c     subset an mht image

      implicit none
      real in(100000),out(100000)
      integer  i,j,len,iac,idn,k,irec,kac,kdn
      character*80 a_input,a_output,c

      if(iargc().lt.7)then
         print *,'usage: subsetmht infile outfile length ac-start dn-start ac-size dn-size'
         stop
      end if
     ...
     +     form='unformatted',recl=kac*8)
      ...
      irec=0
      do i=idn,idn+kdn-1
         read(10,rec=i,err=99) (in(j), j=1,len*2)
         k=0
         do j=iac,iac+kac-1
            k=k+1
            out(k) = in(j)
         ...
         irec=irec+1
         write(11,rec=irec) (out(j),j=1,kac*2)
      end do
