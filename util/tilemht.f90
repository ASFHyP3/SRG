  !c  tilemht - create a tiled file from a list of mht files, take looks
  
  real*4, allocatable :: in(:,:),out(:,:),inlooks(:,:),inline(:),outmag(:),outhgt(:)
...
11 close(21)
  ...
  allocate (inline(len*2),outmag(lenout),outhgt(lenout))
     ...
     do ix=0,nx-1
        ...
        if(ifile.le.nigrams)then
           ...
           do line=1,linesout
              ...
              do ipix=1,lenout
                 outmag(ipix)=sum(inline((ipix-1)*looksx+1:(ipix-1)*looksx+looksx))/looksx/looksy
                 outhgt(ipix)=sum(inline((ipix-1)*looksx+1+len:(ipix-1)*looksx+looksx+len))/looksx/looksy
              end do
              out(ix*lenout+1:ix*lenout+lenout,line)=outmag
              out(ix*lenout+1+lenout*nx:ix*lenout+lenout+lenout*nx,line)=outhgt
           end do
